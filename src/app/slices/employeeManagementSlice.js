...
export const empPersonalsUpdation = createAsyncThunk('empPersonalsUpdation', async (payload, { rejectWithValue }) => {
    try {
        const isDetailsUpdated = await api.patch(`/employees/${payload.employeeToUpdate.id}`, payload.modifiedValuesObj);
        if (isDetailsUpdated) {
...
export const empContactsUpdation = createAsyncThunk('empContactsUpdation', async (payload, { rejectWithValue }) => {
    try {
        const isContactsUpdated = await api.patch(`/employees/${payload.employeeToUpdate.id}`, { contact_details: payload.updEmpContactDetails });
...
export const empAcadRecordUpdation = createAsyncThunk('empAcadRecordUpdation', async (payload, { rejectWithValue }) => {
    try {
        const response = await api.get(`/employees/${payload.empID}`);
        ...
        const empUpdation = await api.patch(`/employees/${payload.empID}`, patchPayload);
        if (empUpdation) {
...
export const empProfRecordUpdation = createAsyncThunk('empProfRecordUpdation', async (payload, { rejectWithValue }) => {
    try {
        const responseEmp = await api.get(`/employees/${payload.employeeID}`);
        ...
        const employeeUpdation = await api.patch(`/employees/${payload.employeeID}`, patchPayloadData);
        if (employeeUpdation) {
...
export const empGuarantorUpdation = createAsyncThunk('empGuarantorUpdation', async (payload, { rejectWithValue }) => {
    try {
        const emp = await api.get(`/employees/${payload.empID}`);
        ...
        const empUpdate = await api.patch(`/employees/${payload.empID}`, { guarantor_details: updatedGuarantors });
        if (empUpdate) {
